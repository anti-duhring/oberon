...
`oberon-grill`, `oberon-sync` and `oberon-handoff` are model-invocable. `oberon-grill` opens by
confirming with the user before any side effect — including the store creation it now owns
(ADR-0015). `oberon-sync` reads the last journal entry first and does nothing when nothing
observable has changed. `oberon-delete` is user-invoked only (ADR-0011).
...
- The gates exist because self-invocation *executes* rather than offers. Unbidden,
  `oberon-grill` would turn a casual design chat into a full interview and, at the first thing
  it settled, mint an id, create a directory and commit. That is a commitment the user should
  make knowingly. One confirmation turn preserves auto-discovery at negligible cost.
